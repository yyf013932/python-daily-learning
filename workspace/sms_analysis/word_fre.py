...
count_df=pd.DataFrame(df_use_group['text'].groupby([df_use_group['good_bad'],df_use_group['get_send']]).count())   
count_df.reset_index(inplace=True)
unique_mem_df=pd.DataFrame(df_use_group['mem_id'].groupby([df_use['good_bad'],df_use['get_send']]).unique()).reset_index()
...
count_df_mem=pd.DataFrame(df_use_group['text'].groupby([df_use_group['mem_id'],df_use_group['get_send'],df_use_group['good_bad']]).count()).reset_index()
...
train_count['good_bad'],train_count['get_send']]).count()).reset_index()

train_count_use.columns=['mem_id','good_bad','get_send','sms_num']

word_fre_df=pd.merge(train_count,train_count_use,how='inner',on=['mem_id','good_bad','get_send'])
'''
...
def get_propo1(word):
    ...
    """
    tem_df=pd.DataFrame(statistic_word_df[word].groupby([statistic_word_df['mem_id'],statistic_word_df['good_bad'],\
    statistic_word_df['get_send']]).mean()).reset_index()
